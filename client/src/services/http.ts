...
async function request<T>(
  ...
  if (!res.ok) {
    throw new HttpError(json.error || `HTTP ${res.status}`, res.status, json.needsReauth);
  }

  return json.data !== undefined ? json.data : json;
}

async function uploadFile<T>(url: string, file: File, signal?: AbortSignal): Promise<T> {
  const token = getToken();
  const headers: Record<string, string> = {};
...
export const http = {
  ...
  upload: <T>(url: string, file: File, signal?: AbortSignal) => uploadFile<T>(url, file, signal),
};
